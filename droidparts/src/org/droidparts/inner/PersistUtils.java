...
public final class PersistUtils implements SQL.DDL {
	...
	public static <T> ArrayList<String> getAddMissingColumns(SQLiteDatabase db, Class<? extends Entity> cls)
		...
		for (FieldSpec<ColumnAnn> spec : columns) {
			...
			if (defaultVal != null) {
				...
				converter.putToContentValues((Class<T>) spec.field.getType(), spec.genericArg1, spec.genericArg2, cv,
	...
	public static String getCreateIndex(String table, boolean unique, String firstColumn, String... otherColumns) {
	...
	public static ArrayList<String> getTableNames(SQLiteDatabase db) {
		return readStrings(db, "SELECT name FROM sqlite_master WHERE type='table'", 0);
	}
	...
	public static String getSQLCreate(String tableName, FieldSpec<ColumnAnn>[] specs) {
		StringBuilder sb = new StringBuilder();
		...
		for (FieldSpec<ColumnAnn> spec : specs) {
			...
			Converter<?> converter = ConverterRegistry.getConverter(spec.field.getType());
	...
	private static void appendForeignKeyDef(FieldSpec<ColumnAnn> spec, StringBuilder sb) {
		Class<? extends Entity> entityType = spec.field.getType().asSubclass(Entity.class);
