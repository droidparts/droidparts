...
public class CustomSerializerJSONTestCase extends AbstractJSONTestCase {
	...
	public static abstract class AbstractAlbum extends Model {
		...
		public static class Converter extends ModelConverter<AbstractAlbum> {
			...
			private final JSONSerializer<NameAlbum> nameAlbumSerializer;
			private final JSONSerializer<YearAlbum> yearAlbumSerializer;

			public Converter() {
				nameAlbumSerializer = new JSONSerializer<NameAlbum>(NameAlbum.class, null);
				yearAlbumSerializer = new JSONSerializer<YearAlbum>(YearAlbum.class, null);
			}
			...
			protected JSONSerializer<? extends AbstractAlbum> getJSONSerializer(Class<AbstractAlbum> valType,
					JSONObject src) {
				if (src.has("name")) {
					return nameAlbumSerializer;
				} else if (src.has("year")) {
					return yearAlbumSerializer;
				} else {
